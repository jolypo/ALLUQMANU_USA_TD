...
class AlpacaProvider:
    def __init__(self):
        self.headers={"APCA-API-KEY-ID": settings.alpaca_api_key, "APCA-API-SECRET-KEY": settings.alpaca_api_secret}
    ...
    async def bars(self, symbol: str, timeframe: str, lookback_days: int) -> pd.DataFrame:
        ...
        data=await self._get(url,{"timeframe":timeframe,"start":start,"adjustment":"all","feed":settings.alpaca_stock_feed,"limit":10000,"sort":"asc"})
        rows=data.get("bars",[])
        if not rows: return pd.DataFrame()
        return pd.DataFrame([{"timestamp":x.get("t"),"open":x.get("o"),"high":x.get("h"),"low":x.get("l"),"close":x.get("c"),"volume":x.get("v")} for x in rows])
    ...
    async def market_clock(self) -> dict:
        url=f"{settings.alpaca_trading_base_url}/clock"
        return await self._get(url)

    async def option_chain(self, underlying: str, min_dte: int, max_dte: int, opt_type: str | None=None) -> dict:
        now=datetime.now(timezone.utc).date()
        params={"feed":settings.alpaca_options_feed,"limit":1000,"expiration_date_gte":str(now+timedelta(days=min_dte)),"expiration_date_lte":str(now+timedelta(days=max_dte))}
        if opt_type: params["type"]=opt_type
    ...
    async def option_quotes(self, contract_symbols:list[str]) -> dict:
        ...
        d=await self._get(url,{"symbols":",".join(contract_symbols[:100]),"feed":settings.alpaca_options_feed})
        return d.get("quotes",{})
